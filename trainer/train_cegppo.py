...
# arXiv: 2509.20712
#
# Clipping mechanism: GRADIENT-PRESERVING with bounded out-of-clip contributions
#
# Standard PPO zeroes out gradients for tokens outside [1-ε, 1+ε]:
#   • PA&LP (positive advantage, large probability): r > 1+ε, A > 0 → zero gradient
#     These are "already-likely" tokens for a good action — PPO ignores them.
#   • NA&LP (negative advantage, low probability): r < 1-ε, A < 0 → zero gradient
#     These are "already-unlikely" tokens for a bad action — PPO ignores them.
#
# CE-GPPO reintroduces these tokens with bounded gradient magnitudes:
#
#   NA&LP: l_t = β1·(1-ε)/sg(r_t) · r_t·A   [gradient = β1·(1-ε)·A · ∇log π_θ]
#   PA&LP: l_t = β2·(1+ε)/sg(r_t) · r_t·A   [gradient = β2·(1+ε)·A · ∇log π_θ]
#   in-clip: l_t = r_t·A                     [standard unclipped surrogate]
#
# Using stop-gradient on the denominator preserves the ratio r_t in the gradient
# direction but bounds its magnitude to β1·(1-ε) or β2·(1+ε) respectively.
#
# Effect on entropy:
#   • NA&LP (β1 < 1): slows down over-exploitation of already-unlikely bad actions
#   • PA&LP (β2 ≥ 1): preserves exploration of already-likely good actions
#   Setting β1=β2=0 recovers PPO (no gradient outside clip).
#   Setting β2=1.0 (default) keeps full exploration gradient for PA&LP tokens.
# ─────────────────────────────────────────────────────────────────────────────
...
def cegppo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # ── CE-GPPO: gradient-preserving objective ────────────────────────────
        # [DIFFER] Tokens outside [1-ε, 1+ε] are not zeroed — they contribute
        # bounded gradients via β1·(1-ε) or β2·(1+ε) scaling factors.
        ratio = torch.exp(per_token_logps - old_per_token_logps)             # [B*G, L]
        adv   = advantages.unsqueeze(1)                                       # [B*G, 1]

        # Base: unclipped r·A for all "otherwise" (in-clip or REINFORCE passthrough) tokens
        per_token_obj = ratio * adv

        # Identify out-of-clip regions
        mask_nalp = (ratio < (1.0 - args.epsilon)) & (adv < 0.0)  # below-clip, negative-adv
        mask_palp = (ratio > (1.0 + args.epsilon)) & (adv > 0.0)  # above-clip, positive-adv

        # NA&LP: β1·(1-ε)/sg(r)·r·A
        # stop_gradient on denominator → gradient magnitude = β1·(1-ε) · A
        nalp_obj = args.beta1_ce * (1.0 - args.epsilon) / ratio.detach() * ratio * adv

        # PA&LP: β2·(1+ε)/sg(r)·r·A
        # stop_gradient on denominator → gradient magnitude = β2·(1+ε) · A
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            # CE-GPPO-specific: fraction of tokens in each gradient-preserved region
            # [DIFFER] Higher nalp/palp fractions = more tokens that GRPO would ignore
            #          but CE-GPPO still updates. This is the algorithm's core contribution.
            nalp_frac = (mask_nalp & m).float().sum().item() / total
            palp_frac = (mask_palp & m).float().sum().item() / total
            preserved = nalp_frac + palp_frac  # total gradient-preserved fraction
            ...
            clip_metrics = {
                **r_stats,
                **c_fracs,   # clipped_fraction etc. (what GRPO would zero — CE-GPPO preserves)
                "clip/nalp_fraction":       nalp_frac,
                "clip/palp_fraction":       palp_frac,
                "clip/preserved_grad_frac": preserved,
            }
            stability = {
            ...
            Logger(
                ...
                f'NA&LP:{nalp_frac:.3f}  PA&LP:{palp_frac:.3f}  Preserved:{preserved:.3f}  '
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_weight",     type=str,   default="cegppo")
    ...
    # ── CE-GPPO-specific: gradient-preserving parameters ─────────────────────
    # [DIFFER] ε is the same clip boundary as GRPO, but out-of-clip gradients
    # are PRESERVED (not zeroed) with bounded magnitudes.
    parser.add_argument("--epsilon",   type=float, default=0.2,
                        help="Clip boundary. Same meaning as GRPO's ε — identifies NA&LP and PA&LP tokens.")
    parser.add_argument("--beta1_ce", type=float, default=0.75,
                        help="Gradient scale for NA&LP tokens. 0=PPO, <1=reduced, >1=amplified. "
                             "Lower values slow entropy increase from below-clip negative updates.")
    parser.add_argument("--beta2_ce", type=float, default=1.0,
                        help="Gradient scale for PA&LP tokens. 0=PPO, 1=full gradient (paper default). "
                             "1.0 fully preserves exploration gradient for above-clip positive updates.")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(project=args.wandb_project,
                   name=f"CEGPPO-b1ce{args.beta1_ce}-b2ce{args.beta2_ce}-LR{args.learning_rate}",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        cegppo_train_epoch(epoch, loader, n_iters, rollout_engine, ref_model, reward_model,
