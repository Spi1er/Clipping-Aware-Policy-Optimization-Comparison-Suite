...
#
# Key innovations:
#   1. Clip-Higher: asymmetric clipping [1 - epsilon_low, 1 + epsilon_high]
#      to avoid entropy collapse while keeping stability.
#   2. Token-Level Loss: normalize by TOTAL tokens across the group (not
#      per-sequence mean), so long responses don't dominate.
#   3. Dynamic Sampling: skip groups where all rewards are identical (zero
#      advantage → zero gradient). Saves compute and reduces noise.
#
# Loss:  L_t = min(r_t * A, clip(r_t, 1-eps_low, 1+eps_high) * A) - β·KL
# Aggregation:  mean over (sum_per_group / total_tokens_in_group)
# ---------------------------------------------------------------------------
...
def dapo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model, start_step=0, wandb=None):
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # ---- DAPO: Dynamic Sampling (opt-in via --dynamic_sampling) ----
        # Skips groups where all rewards are identical (zero advantage → zero gradient).
        # Disabled by default so all methods see identical batches for fair comparison.
        grouped_rewards = rewards.view(-1, args.num_generations)  # [B, G]
        if args.dynamic_sampling:
            valid_groups = grouped_rewards.std(dim=1) > 1e-6  # [B] bool
        ...
        # Clip-Higher: asymmetric clipping bounds
        ...
        # Loss aggregation: per-sequence mean → batch mean (identical to original train_grpo.py)
        # so that all methods have the same effective gradient scale and LR is comparable.
        policy_loss = -((per_token_obj * completion_mask).sum(dim=1) / completion_mask.sum(dim=1).clamp(min=1)).mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='dapo', type=str)
    ...
    parser.add_argument("--epsilon_low",  type=float, default=0.2,  help="Lower clip bound (keeps stability)")
    parser.add_argument("--epsilon_high", type=float, default=0.28, help="Upper clip bound (higher = more exploration)")
    # Dynamic Sampling: off by default so all methods see the same batches (fair comparison).
    # Enable with --dynamic_sampling to get DAPO's full speed benefit at the cost of comparability.
    parser.add_argument("--dynamic_sampling", action="store_true",
                        help="Skip zero-advantage groups (DAPO paper feature). Disable for fair cross-method comparison.")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-DAPO")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(project=args.wandb_project,
                   name=f"DAPO-Epoch{args.epochs}-BS{args.batch_size}-LR{args.learning_rate}",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0:
            ...
            dapo_train_epoch(epoch, loader, len(loader)+skip, rollout_engine, ref_model, reward_model,
                             start_step, wandb)
        else:
            dapo_train_epoch(epoch, loader, len(loader), rollout_engine, ref_model, reward_model,
                             0, wandb)
