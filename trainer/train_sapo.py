...
# arXiv: 2511.20347
#
# Clipping mechanism: SOFT SIGMOID GATE (replaces hard clip entirely)
#
# GRPO/PPO: L = min(r·Â, clip(r,1±ε)·Â)
#   → hard zero-gradient regions beyond the clip boundary
#
# SAPO: L = f(r) · Â  where  f(r) = σ(τ·(r−1)) · 4/τ
#   → smooth, continuously differentiable gate
#   → near on-policy (r≈1): f(r) ≈ 2/τ → gradient ≈ 2/τ · Â (controllable scale)
#   → far off-policy: sigmoid saturates → gradient smoothly attenuated to 0
#
# Asymmetric temperature:
#   τ = τ_pos  when A > 0  (shallower gate → wider trust region for good tokens)
#   τ = τ_neg  when A ≤ 0  (steeper gate  → tighter trust region for bad tokens)
#   Setting τ_neg > τ_pos reduces early training collapse from negative-advantage
#   gradient variance, without the hard dead-zone of clipping.
#
# Key observable difference from hard-clip methods:
#   clip/avg_gate        — mean f(r) value; measures "how much of the update is gated"
#   clip/gate_saturation_frac — fraction of tokens where f(r) is near its max (2/τ)
#                               vs near saturation; low = SAPO is working as a gate
# ─────────────────────────────────────────────────────────────────────────────
...
def sapo_gate(ratio: torch.Tensor, adv: torch.Tensor,
    ...
    """
    SAPO soft gate: f(r) = σ(τ·(r−1)) · 4/τ with per-token asymmetric temperature.

    At r=1: f(1) = σ(0)·4/τ = 0.5·4/τ = 2/τ  (scales linearly with 1/τ)
    As r→∞: f → 4/τ  (saturated)
    As r→0: f → 0    (suppressed)

    The 4/τ scale factor is chosen so that the gate weight w(r) = ∂f/∂r|_{r=1}
    = τ·σ(0)·(1-σ(0))·4/τ = 1, matching the unclipped gradient magnitude at r=1.
    """
...
def sapo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # ── SAPO-specific: soft sigmoid gate ─────────────────────────────────
        # [DIFFER] Replaces min(r·Â, clip(r)·Â) with f(r)·Â.
        # f(r) is always differentiable; no dead zones.
        ratio = torch.exp(per_token_logps - old_per_token_logps)             # [B*G, L]
        adv   = advantages.unsqueeze(1)                                       # [B*G, 1]

        # Gate uses asymmetric temperature per token based on advantage sign
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            # SAPO-specific gate metrics
            avg_gate_val   = (gate * completion_mask).sum().item() / (completion_mask.sum().item() + 1e-8)

            # Gate "saturation" = fraction of tokens where gate is near its maximum 2/τ
            # (i.e., where the sigmoid is well past its midpoint → acting like full update)
            # Threshold: gate > 0.9 * (2/tau), sign-resolved per token
            tau_arr   = torch.where(adv.expand_as(ratio) > 0,
                                    torch.full_like(ratio, args.tau_pos),
                                    torch.full_like(ratio, args.tau_neg))
            gate_max  = 4.0 / tau_arr        # maximum possible gate value = 4/τ
            gate_sat  = (gate > 0.9 * gate_max / 2.0)  # near top of useful range
            sat_frac  = (gate_sat & m).float().sum().item() / (m.float().sum().item() + 1e-8)
            ...
            clip_metrics = {
                **r_stats,
                "clip/avg_gate":           avg_gate_val,
                "clip/gate_saturation_frac": sat_frac,
            }
            stability = {
            ...
            Logger(
                ...
                f'AvgGate:{avg_gate_val:.4f}  GateSat:{sat_frac:.3f}  '
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_weight",     type=str,   default="sapo")
    ...
    # ── SAPO-specific: asymmetric sigmoid gate temperatures ───────────────────
    # [DIFFER] No ε parameter — the gate sharpness τ plays the role of the trust region.
    # τ_neg > τ_pos creates sharper gate for negative-advantage tokens.
    parser.add_argument("--tau_pos", type=float, default=2.0,
                        help="Gate temperature for A>0 tokens. Smaller τ = wider trust region. "
                             "f(r)=σ(τ·(r-1))·4/τ.  At r=1: gradient weight = 1 regardless of τ.")
    parser.add_argument("--tau_neg", type=float, default=5.0,
                        help="Gate temperature for A≤0 tokens. Larger τ = tighter gate. "
                             "τ_neg > τ_pos reduces variance from negative-advantage gradients.")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(project=args.wandb_project,
                   name=f"SAPO-tauP{args.tau_pos}-tauN{args.tau_neg}-LR{args.learning_rate}",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        sapo_train_epoch(epoch, loader, n_iters, rollout_engine, ref_model, reward_model,
